...
# ⚠️ CoAPClient import SUPPRIMÉ - Les appels CoAP directs sont désactivés
# from lib.coap.client import CoAPClient
# Les fonctions de parsing sont conservées pour RECEVOIR des messages CoAP (serveur uniquement)
...
# Réduire les logs Flask (removed - using print() instead)
...
class CoAPServer:
    ...
    ⚠️ ARCHITECTURE CoAP:
    - Ce serveur REÇOIT uniquement les messages CoAP des nodes (mode serveur)
    - Il NE DOIT PAS envoyer de requêtes CoAP directes (mode client DÉSACTIVÉ)
    - Toutes les commandes aux nodes passent par les Border Routers en WebSocket:
        Python Server → WebSocket → Border Router → CoAP → Thread Node

    Les appels directs CoAP (send_coap_post) sont bloqués pour garantir cette architecture.
    """
    ...
    def send_coap_post(self, address, uri_path, payload):
        """
        ⚠️ DÉSACTIVÉ - Les appels CoAP directs sont interdits!

        Cette méthode a été désactivée pour garantir que TOUTES les communications
        ...
        """
        print(f"❌ APPEL CoAP DIRECT BLOQUÉ: {address}/{uri_path} - '{payload}'")
        print(f"   ⚠️  Utilisez send_command_via_br() ou le Border Router WebSocket!")
        return False
    ...
    def send_command_via_br(self, node_name, command_type, payload=None):
        ...
        try:
            ...
            if success:
                print(f"📤 Commande {command_type} envoyée au BR {br_id} pour node {node_name} (request_id: {request_id})")
    ...
    def handle_battery_event_from_br(self, data):
        ...
        print(f"🔋 Batterie depuis BR {br_id}, node {node_name}: {voltage}V ({percentage}%)")
    ...
    def handle_ble_event_from_br(self, data):
        ...
        detection_data = {
            'node': node_name,
            'br_id': br_id,
            'device': ble_addr,  # Changé de 'ble_addr' à 'device' pour correspondre au JS
            'rssi': rssi,
            'code': code,
            'timestamp': datetime.now().isoformat()
        }
        ...
        while self.demo_mode and self.running:
            try:
                ...
                if current_time - last_announce >= announce_interval:
                    # ⚠️ DÉSACTIVÉ - Les appels CoAP directs sont interdits
                    print(f"\r[{datetime.now().strftime('%H:%M:%S')}] ⚠️  Annonce serveur DÉSACTIVÉE (CoAP direct bloqué)")
                    # NOTE: Cette fonctionnalité doit être réimplémentée via Border Router WebSocket
                    # # Annonce silencieuse
                    # addresses = self.registry.get_all_addresses()
                    # if addresses:
                ...
                # Flash continu des LEDs rouges - ⚠️ DÉSACTIVÉ (CoAP direct bloqué)
                # NOTE: Cette fonctionnalité doit être réimplémentée via Border Router WebSocket
                # addresses = self.registry.get_all_addresses()
                # if addresses:
                #     # Créer un socket temporaire pour l'envoi
...
def scan_all_nodes_via_brs():
    """
    Orchestrateur central du scan réseau via Border Routers (découverte dynamique)

    Cette fonction coordonne le scan de tous les nodes du réseau en:
    1. Récupérant la liste des Border Routers connectés
    2. Envoyant une commande 'scan_all_nodes' à CHAQUE BR
    3. Chaque BR découvre automatiquement les nodes dans son réseau Thread
    4. Les résultats sont reçus de manière asynchrone via WebSocket

    Architecture de découverte dynamique:
    - Python ne connait PAS la liste des nodes à l'avance
    - Python ne sait PAS quel node est derrière quel BR
    - Chaque BR découvre ses propres nodes via OpenThread APIs
    - Chaque BR scanne ses nodes et renvoie les résultats

    Les résultats sont reçus de manière asynchrone via WebSocket et
    émis aux clients web via Socket.IO (événement 'scan_node_result')

    Returns:
        dict: Statistiques du scan (broadcast_sent, br_count)
    """
    try:
        # 1. Récupérer tous les Border Routers connectés
        active_brs = native_ws_handler.get_active_br_ids()

        if not active_brs:
            print("⚠️ No Border Routers connected - cannot scan network")
            return {
        ...
        print(f"🔍 Starting dynamic network scan: broadcasting to {len(active_brs)} Border Router(s)")

        # 2. Envoyer commande scan_all_nodes à TOUS les BRs
        commands_sent = 0
        for br_id in active_brs:
            ...
            if success:
                commands_sent += 1
                print(f"✅ scan_all_nodes broadcast to {br_id} (request_id: {request_id})")
            else:
                print(f"❌ Failed to send scan_all_nodes to {br_id}")

        print(f"📊 Dynamic scan broadcast complete: {commands_sent}/{len(active_brs)} BRs notified")
        return {
            'broadcast_sent': True,
        ...
        print(f"❌ Error in scan_all_nodes_via_brs: {e}")
        import traceback
        traceback.print_exc()
        return {
            'broadcast_sent': False,
...
def get_br_status():
    ...
    try:
        ...
        print(f"Erreur récupération statut BR: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
def get_border_routers():
    """
    Alias pour /api/br/status - compatibilité avec network-map.html
...
def trigger_scan():
    """
    Déclenche un scan complet du réseau via les Border Routers

    Cette route lance un scan orchestré de tous les nodes du réseau.
    Les résultats sont envoyés de manière asynchrone via Socket.IO
    (événement 'scan_node_result') au fur et à mesure que les nodes
    répondent.
    ...
    """
    try:
        print("🔍 API: Triggering network scan via BRs")

        # Lancer le scan orchestré (découverte dynamique)
...
def handle_br_connect():
    ...
    if not br_id or not auth_token:
        print("❌ Connexion BR refusée: br_id ou auth_token manquant")
        return False

    # Vérifier l'authentification
    if BR_AUTH_ENABLED and not verify_br_token(br_id, auth_token):
        print(f"❌ Connexion BR refusée: authentification échouée pour {br_id}")
        return False
    ...
    if success:
        ...
        print(f"✅ Border Router {br_id} connecté via WebSocket (sid: {request.sid})")
        return True
    else:
        print(f"❌ Échec enregistrement BR {br_id}")
        return False
...
def handle_node_event(data):
    ...
    try:
        ...
        if not br_id or not node_name or not event_type:
            print("Événement node invalide: champs manquants")
            return
        ...
        print(f"Erreur traitement événement node: {e}")
...
def handle_command_response(data):
    ...
    try:
        ...
        print(f"📨 Réponse commande {request_id}: {status} (node: {node_name}, BR: {br_id})")

    except Exception as e:
        print(f"Erreur traitement réponse commande: {e}")
...
def handle_topology_update(data):
    ...
    try:
        ...
        print(f"🗺️ Topologie mise à jour pour BR {br_id}: {len(node_names)} nodes")
        ...
        print(f"Erreur mise à jour topologie: {e}")
